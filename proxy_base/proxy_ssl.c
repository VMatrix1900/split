...
struct cert_ctx *load_cert_ctx()
{
  ...
  char cert_buffer[] =
      ...
      "-----END CERTIFICATE-----\n";
  struct cert_ctx *ctx = malloc(sizeof(struct cert_ctx));
  memset(ctx, 0, sizeof(struct cert_ctx));
  ctx->cacrt = load_certificate(cert_buffer);
  if (!ctx->cacrt) {
    printf("certf load error\n");
    return NULL;
  } else {
    char *ca_subject = ssl_x509_subject(ctx->cacrt);
    /* printf("Loaded CA: %s\n", ca_subject); */
    free(ca_subject);
  }
  ctx->cakey = load_key(pkey);
  ssl_x509_refcount_inc(ctx->cacrt);
  sk_X509_insert(ctx->chain, ctx->cacrt, 0);
  fprintf(stderr, "chain num:%d\n", sk_X509_num(ctx->chain));
  if (!ctx->cakey) {
    printf("keyf load error\n");
    return NULL;
  }
  if (X509_check_private_key(ctx->cacrt, ctx->cakey) != 1) {
    printf("CA cert does not match key.\n");
    return NULL;
  } else {
  }
  ctx->key = ssl_key_genrsa(1024);
  if (!ctx->key) {
