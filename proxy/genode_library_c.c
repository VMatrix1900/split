...
#define CEHCK_FD if(sharedmem_fd < 0){ printf("fd must be larger than 0\n"); return -1;}
...
void circular_queue_initialize(int size)
{
	...
	sharedmem_to_genode_ch = (sharedmem_buffer_base*)malloc(sizeof(sharedmem_buffer_base));
	sharedmem_to_linux_ch = (sharedmem_buffer_base*)malloc(sizeof(sharedmem_buffer_base));
...
void circular_queue_init_private(sharedmem_buffer_base* bb)
{
	...
	if(bb->buffer != NULL)
		free(bb->buffer);
	bb->buffer = (sharedmem_packet_info*)
			malloc(sizeof(sharedmem_packet_info) * bb->size);
}
...
int circular_queue_push_private(sharedmem_buffer_base* bb, sharedmem_packet_info pi)
{
	...
	if((bb->read_head - 1 == bb->write_head) ||
	   (bb->read_head == 0 && bb->write_head == bb->size-1))//if the queue if full
	{
		return -1;
	}
...
{
	return sharedmem_addr_buf_size;
}
...
unsigned long * sharedmem_get_addr()
{
	...
	ssize_t read_data_cnt = sharedmem_read_from_device(sharedmem_buf, sharedmem_addr_buf_size);
	sscanf(sharedmem_buf, "%lx\n%lx\n", &recv_buf_addr, &recv_buf_phy_addr);
	printf("Buffer:[%lx] Phy:[%lx][size: %d]\n", recv_buf_addr, recv_buf_phy_addr, read_data_cnt);
...
void sharedmem_run_channel_test()
{
	...
	for(i=0; i<100; i++)
	{
		...
		sharedmem_packet_info temp = circular_queue_pop_private(sharedmem_to_genode_ch);
...
int sharedmem_write_into_device(const char* command, int size)
{
	...
	if(written >= 0)
		;//printf("written data: %d\n", written);  // handle successful write (which might be a partial write!)
	else
		printf("write_into_device:: data isn't written\n");  // handle real error
...
unsigned long * sharedmem_get_mem_map(long unsigned mem_addr)
{
	...
	//map= (long unsigned *)(mmap(0,_addr_buf_size, PROT_READ|PROT_WRITE,MAP_SHARED,_fdmem,mem_addr));

	sharedmem_map = (long unsigned *)
			(mmap(0, sharedmem_addr_buf_size,
				  PROT_READ|PROT_WRITE, MAP_SHARED,
				  sharedmem_fdmem, mem_addr));

	printf("Mapped: virt[0x%0lx] -> phy[0x%0lx]\n",(long unsigned)(sharedmem_map), (long unsigned)mem_addr);
	//printf("content: 0x%0lx[0x%0lx]\n",(long unsigned)(*(map)), (long unsigned)map);
	/*
	//TEST
	while(1)
	{
		...
		printf("content: 0x%0lx[0x%0lx]\n",(long unsigned)(*(map)), (long unsigned)map);
		//printf("content: 0x%0lx[0x%0lx]\n",*((long unsigned*)mem_addr), mem_addr);
		sleep(5);
	}
	*/
