                    ...
                    struct sockaddr *peeraddr, int peeraddrlen, void *ptr)
{
    ...
    ctx->cli_shm_ctx = malloc(
        sizeof(shm_ctx_t));  // act as a client, receive msg from the server
    ctx->serv_shm_ctx = malloc(
        sizeof(shm_ctx_t));  // act as a server, receive msg from the client
