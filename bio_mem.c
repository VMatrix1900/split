...
int krx_ssl_init(
    krx* k, int isserver,
    info_callback cb);        /* init the SSL* (the "connection"). we use the
                                 `isserver` to tell SSL that it should either use the
                                 server or client protocol */
int krx_ssl_shutdown(krx* k); /* cleanup SSL allocated mem */
int krx_ssl_verify_peer(
    int ok, X509_STORE_CTX* ctx); /* we set the SSL_VERIFY_PEER option on the
                                     SSL_CTX, so that the server will request
                                     the client certificate. We can use the
...
int krx_ssl_ctx_init(krx* k, const char* keyname)
{
    ...
    k->ctx = SSL_CTX_new(TLSv1_2_method());
    if (!k->ctx) {
    ...
    /*r = SSL_CTX_set_cipher_list(k->ctx,
...
int krx_ssl_verify_peer(int ok, X509_STORE_CTX* ctx) { return 1; }
